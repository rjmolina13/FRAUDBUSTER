...
class DynamicLearningEngine {
  ...
  initializeWithDefaults() {
    ...
    this.modelState.domainBiases.set('jobstreet.com', 0.85);
  ...
  /**
   * Get learning metrics for UI/debugging consumers
   */
  async getMetrics() {
    try {
      const totalFeedback = this.performanceMetrics.totalSamples || this.feedbackBuffer.length || 0;
      const accuracyRate = this.performanceMetrics.accuracy || 0;
      const patternsLearned = (this.modelState.patternAccuracy && typeof this.modelState.patternAccuracy.size === 'number')
        ? this.modelState.patternAccuracy.size
        : 0;
      const lastUpdate = this.modelState.lastUpdate || null;
      return { totalFeedback, accuracyRate, patternsLearned, lastUpdate };
    } catch (error) {
      return { totalFeedback: 0, accuracyRate: 0, patternsLearned: 0, lastUpdate: null, error: error.message };
    }
  }

  /**
   * Compute confidence adjustment using classification result, aggregated fraud stats, and learned biases
   * @param {Object} classification - Result from PageContextAnalyzer.classifyPage(...)
   * @param {Object} context - { fraudPercentage, avgConfidence, domain }
   * @returns {number} adjustment in [-0.30, 0.30]
   */
  async getConfidenceAdjustment(classification, context = {}) {
    try {
      const pageType = (classification && classification.pageType) ? classification.pageType : 'unknown';
      const clfConfidence = (classification && typeof classification.confidence === 'number') ? classification.confidence : 0.5;
      const scores = (classification && classification.scores) || {};
      const fraudPercentage = (context && typeof context.fraudPercentage === 'number') ? context.fraudPercentage : 0;
      const avgConfidence = (context && typeof context.avgConfidence === 'number') ? context.avgConfidence : clfConfidence;
      const domain = context && context.domain ? context.domain : null;

      const thresholds = this.modelState && this.modelState.classificationThresholds
        ? this.modelState.classificationThresholds
        : { analysisSkipThreshold: 0.8, confidenceThreshold: 0.7 };

      let adjustment = 0;

      // Domain bias value (can be number or object with { bias })
      let biasValue = 0;
      try {
        const biasEntry = domain ? this.modelState.domainBiases.get(domain) : null;
        biasValue = typeof biasEntry === 'number' ? biasEntry : (biasEntry && typeof biasEntry.bias === 'number' ? biasEntry.bias : 0);
      } catch (e) {
        biasValue = 0;
      }

      if (pageType === 'landing_page') {
        // Down-adjust confident landing pages to avoid false positives
        if (clfConfidence >= (thresholds.analysisSkipThreshold || 0.8)) {
          adjustment -= 0.20;
        } else if (clfConfidence >= (thresholds.confidenceThreshold || 0.7)) {
          adjustment -= 0.10;
        } else {
          adjustment -= 0.05;
        }

        // Positive domain bias: likely legitimate aggregator -> stronger down adjustment
        if (biasValue > 0) {
          adjustment -= Math.min(0.10, biasValue * 0.10);
        } else if (biasValue < 0) {
          // Problematic domain: ease the down adjustment slightly
          adjustment += Math.min(0.05, Math.abs(biasValue) * 0.10);
        }

        // Counteract if fraud evidence is high
        if (fraudPercentage >= 50) {
          adjustment += 0.10;
        } else if (fraudPercentage === 0) {
          adjustment -= 0.05;
        }
      } else if (pageType === 'job_posting') {
        // Strengthen when feature signals indicate a single posting
        const strengthScore =
          (scores.jobIndicators || 0) * 0.5 +
          (scores.semanticScore || 0) * 0.4 +
          (scores.structureScore || 0) * 0.3 -
          (scores.landingPageScore || 0) * 0.5 -
          (scores.navigationScore || 0) * 0.3;

        if (avgConfidence < 0.6 && strengthScore > 0.5) {
          adjustment += 0.10;
        } else if (avgConfidence > 0.8) {
          adjustment += 0.05;
        }

        // Be conservative with low observed fraud
        ...
        // Domain bias: very safe aggregators -> nudge down; problematic -> nudge up
        if (biasValue > 0.5) {
          adjustment -= 0.05;
        } else if (biasValue < -0.1) {
          adjustment += 0.05;
        }
      } else {
        // Unknown page type: favor caution slightly
        adjustment -= 0.02;
      }
      ...
      console.warn('getConfidenceAdjustment failed:', error);
      return 0;
    }
  }
