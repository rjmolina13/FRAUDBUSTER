...
class FirestoreFalsePositiveIntegration {
  ...
  async storePageClassification(classificationResult) {
    try {
      const cid = classificationResult.classificationId || this.simpleHash(`${classificationResult.url}-${classificationResult.pageType}-${classificationResult.confidence}-${Date.now()}`);
      const ts = classificationResult.timestamp ? new Date(classificationResult.timestamp) : new Date();
      const doc = {
        url: classificationResult.url || '',
        domain: classificationResult.url ? new URL(classificationResult.url).hostname : '',
        page_type: classificationResult.pageType || '',
        confidence: typeof classificationResult.confidence === 'number' ? classificationResult.confidence : 0,
      ...
      console.log('Page classification stored:', cid);
    } catch (error) {
