/**
 * FraudBuster Popup UI Controller
 * Controls popup lifecycle, session-aware loading screen, and user interactions.
 * Responsibilities:
 * - Restore per-tab and per-domain state
 * - Orchestrate step-by-step initialization with progress UI
 * - Toggle extension activation per domain
 * - Display analysis results, community section, and theming
 */
...
class FraudBusterPopup {
  ...
  async init() {
    ...
    try {
      ...
      if (shouldShowLoadingScreen) {
        ...
        const y=document.getElementById('copyrightYear'); if(y){ y.textContent = new Date().getFullYear(); }
  ...
  isSupportedContentScriptUrl(url) {
    try {
      const u = new URL(url);
  ...
  async restoreTabState() {
    ...
    try {
      ...
      if (!tabState) {
        const result = await chrome.storage.session.get([`tabState_${this.currentTab.id}`, `tabActive_${this.currentTab.id}`]);
        tabState = result[`tabState_${this.currentTab.id}`];
        const tabActive = result[`tabActive_${this.currentTab.id}`];
        if (typeof tabActive === 'boolean') {
          this.isExtensionActive = tabActive;
        }
      }
  ...
  setupEventListeners() {
    ...
    extensionToggle.addEventListener('change', (e) => {
      ...
      // Persist per-tab active state for this tab
      if (this.currentTab && typeof this.currentTab.id === 'number') {
        chrome.storage.session.set({ [`tabActive_${this.currentTab.id}`]: this.isExtensionActive }).catch(() => {});
      }
    ...
    document.addEventListener('keydown', (e) => {
      ...
      adminBuffer = (adminBuffer + k.toLowerCase()).slice(-5);
    ...
    const exportBtn = document.getElementById('exportDatasetButton');
    if (exportBtn) {
      exportBtn.addEventListener('click', async () => {
        try {
          document.getElementById('adminStatus').textContent = 'Exporting...';
          const resp = await chrome.runtime.sendMessage({ action: 'adminFullScanExport', tabId: this.currentTab?.id, pageUrl: this.currentTab?.url });
          if (resp && resp.success) {
            document.getElementById('adminStatus').textContent = 'Exported';
          } else {
            document.getElementById('adminStatus').textContent = 'Failed';
          }
        } catch (err) {
  ...
  displayResults(results) {
    ...
    const isAggregated = !!(fraudAnalysis && typeof fraudAnalysis.method === 'string' && fraudAnalysis.method.includes('aggregated_analysis'));
    if (isAggregated) {
      ...
      const fp = typeof fraudAnalysis.fraudPercentage === 'number' ? fraudAnalysis.fraudPercentage : 0;
      riskScore = Math.round(fp);
    ...
    const truncatedCount = (this.jobPostings || []).filter(j => j.truncated).length;
    if (truncatedCount > 0) {
      riskDescription.textContent = 'Some postings have truncated descriptions (See More detected)';
    }
    ...
    if ((fraudAnalysis.method && fraudAnalysis.method.includes('aggregated_analysis')) || fraudAnalysis.method === 'ml_prediction' || fraudAnalysis.method === 'ml_model') {
    ...
    try {
      const individualResults = Array.isArray(fraudAnalysis.individualResults) ? fraudAnalysis.individualResults : [];
      if (individualResults.length > 0) {
        console.log('=== INDIVIDUAL ANALYSIS DETAILS ===');
        individualResults.forEach((r, idx) => {
          const phrases = Array.isArray(r.detectedPhrases) ? r.detectedPhrases.map(p => p.phrase) : [];
          console.log(`Posting ${idx + 1}: isFraud=${!!r.isFraud}, confidence=${(r.confidence*100).toFixed(1)}%`);
          if (phrases.length > 0) console.log('Detected phrases:', phrases.slice(0, 20));
          if (Array.isArray(r.reasons) && r.reasons.length > 0) console.log('Reasons:', r.reasons);
        });
      }
      if (Array.isArray(this.jobPostings) && this.jobPostings.length > 0) {
        console.log('=== PAGE ELEMENTS PROCESSED ===');
        const suspiciousTerms = ['guaranteed income','easy money','quick cash','$','₱','registration fee','processing fee','training fee','equipment fee','deposit','advance payment','membership fee','placement fee','urgent','immediate start','whatsapp','telegram','viber','wechat','signal','gmail','yahoo'];
        this.jobPostings.forEach((job, idx) => {
          const pathInfo = { elementPath: job.elementPath || '', titlePath: job.titlePath || '', companyPath: job.companyPath || '' };
          const desc = (job.description || '').toLowerCase();
          const matches = suspiciousTerms.filter(t => desc.includes(t.toLowerCase()));
          console.log(`Job ${idx + 1}: title="${(job.title||'').substring(0,80)}" company="${job.company||''}"`);
          console.log('Element paths:', pathInfo);
          if (matches.length > 0) console.log('Suspicious matches:', matches);
        });
      }
    } catch (e) {
      console.warn('Verbose logging failed:', e);
    }
  }
  ...
  async detectJobPostings() {
    ...
    try {
      if (!this.isSupportedContentScriptUrl(this.currentTab.url)) {
        this.showError('This page type is not supported for content scripts.');
        this.jobPostings = [];
  ...
  async ensureContentScriptInjected() {
    try {
      if (!this.isSupportedContentScriptUrl(this.currentTab.url)) {
        throw new Error('Unsupported page for content scripts');
      }
...
async function fbLoadingStart() {
  const p = window.fraudBusterPopupInstance || new FraudBusterPopup();
  p.showLoadingScreen();
  await p.updateLoadingState('Initializing extension...', 0, 'step1');
  await p.updateLoadingState('Connecting to database...', 25, 'step2');
  await p.updateLoadingState('Downloading fraud patterns...', 50, 'step3');
  await p.updateLoadingState('Ready!', 100, 'step4');
}

function fbLoadingResume() {
  const p = window.fraudBusterPopupInstance || new FraudBusterPopup();
  p.hideLoadingScreen();
}

function fbLoadingSet(message, progress, step) {
  const p = window.fraudBusterPopupInstance || new FraudBusterPopup();
  p.showLoadingScreen();
  p.updateLoadingState(message, progress, step);
}

window.fbLoadingStart = fbLoadingStart;
window.fbLoadingResume = fbLoadingResume;
window.fbLoadingSet = fbLoadingSet;
