...
if (typeof module !== 'undefined' && module.exports) {
  ...
  /**
   * Convenience wrapper to classify using pageData from content script
   * @param {Object} pageData - Data object extracted by content script
   * @param {string} pageUrl - Current page URL
   * @param {Object} options - Additional options
   * @returns {Object} Classification result compatible with background expectations
   */
  PageContextAnalyzer.prototype.classifyPage = async function(pageData, pageUrl, options = {}) {
    try {
      if (!pageData) {
        throw new Error('classifyPage: pageData is required');
      }
      const pf = pageData.pageFeatures || {};
      const ds = pf.domStructure || {};
      const nav = pf.navigationScore || {};
      const sem = pf.semanticContent || {};
      const elementCounts = pageData.elementCounts || {};

      const totalElements = ds.totalElements || Object.values(elementCounts).reduce((sum, val) => sum + (typeof val === 'number' ? val : 0), 0);
      const textElements = (ds.paragraphCount || elementCounts.paragraphs || 0) + (ds.headingCount || (Array.isArray(pageData.headings) ? pageData.headings.length : 0));
      const navigationElements = nav.navigationElements || 0;
      const semanticElements = sem.semanticElements || {};
      const contentElements = (semanticElements.articles || 0) + (semanticElements.sections || 0) + (semanticElements.mains || 0) + (ds.sectionCount || 0);
      const scriptElements = elementCounts.scripts || 0;
      const styleElements = 0; // Not available from content script

      // Use textContent when available; fallback to text or title
      const pageContent = (pageData.textContent || pageData.text || '') + ' ' + (pageData.title || '');
      const domStructure = {
        totalElements,
        textElements,
        navigationElements,
      ...
      const result = await this.classifyPageContext(pageContent, domStructure, pageUrl, options);

      // Attach scores for consumers that expect a scores object
      result.scores = {
        contentDensity: result.features && typeof result.features.contentDensity === 'number' ? result.features.contentDensity : 0,
        jobIndicators: result.features && typeof result.features.jobIndicators === 'number' ? result.features.jobIndicators : 0,
        navigationScore: result.features && typeof result.features.navigationScore === 'number' ? result.features.navigationScore : 0,
        urlScore: result.features && typeof result.features.urlScore === 'number' ? result.features.urlScore : 0,
        semanticScore: result.features && typeof result.features.semanticScore === 'number' ? result.features.semanticScore : 0,
        structureScore: result.features && typeof result.features.structureScore === 'number' ? result.features.structureScore : 0,
        landingPageScore: result.features && typeof result.features.landingPageScore === 'number' ? result.features.landingPageScore : 0
      };

      return result;
    } catch (error) {
      console.error('Error in classifyPage:', error);
      return {
        pageType: 'unknown',
        confidence: 0.5,
        shouldAnalyze: true,
