...
class FraudDetector {
  ...
  classifyPageType(pageContent, pageUrl = null) {
    ...
    if (jobPostingConfidence > 0.6) {
      ...
      shouldAnalyze = true;
    } else if (landingPageConfidence > 0.7) {
      ...
      shouldAnalyze = jobPostingConfidence > 0.55;
    }
  ...
  initializePatterns(metadata) {
    ...
    this.fraudPatterns = {
      ...
      suspiciousCompany: {
        keywords: ['brand new company', 'unregistered company', 'no company website'],
        weight: 0.2
      }
    };
    ...
    this.legitimateIndicators = {
      ...
      equalOpportunity: {
        keywords: ['equal opportunity employer', 'eeo', 'affirmative action'],
        weight: -0.6
      },
      atsIndicators: {
        keywords: ['workday', 'greenhouse', 'lever', 'icims', 'successfactors', 'taleo'],
        weight: -0.5
      },
      companyAddress: {
        keywords: ['suite', 'street', 'ave', 'avenue', 'road', 'rd', 'blvd', 'boulevard'],
        weight: -0.4
      }
    };
  ...
  async simulateMLPrediction(jobText, modelData) {
    ...
    const highRiskPatterns = [
      ...
      { pattern: /\$\d{4,}.*(week|month)|₱\s?\d{5,}.*(month|week|day|hour)|php\s?\d{5,}.*(month|week|day|hour)|easy.*money|quick.*cash/gi, weight: 0.12, name: 'unrealistic_pay', category: 'Unrealistic Compensation' },
      { pattern: /(registration.*fee|processing.*fee|training.*fee|equipment.*fee|application.*fee|membership.*fee|placement.*fee|security.*deposit|advance.*payment|initial.*payment|startup.*fee)/gi, weight: 0.18, name: 'upfront_fees', category: 'Upfront Payment Requests' },
      ...
      { pattern: /whatsapp|telegram|viber|wechat|signal|personal.*email|gmail.*(contact|email)|yahoo.*(contact|email)|dm\s*me|text\s*me\s*at/gi, weight: 0.09, name: 'suspicious_contact', category: 'Suspicious Communication' },
    ...
    const optimalThreshold = Math.min(0.5, Math.max(0.3, modelData.metadata?.optimal_threshold || 0.5));
    ...
    const hasCritical = featureMatches.some(f => f.feature === 'upfront_fees' || f.feature === 'unrealistic_pay' || f.feature === 'suspicious_contact');
    const isFraud = hasCritical ? true : (fraudScore > optimalThreshold);
  ...
  async performComprehensiveAnalysis(jobText, pageUrl = null) {
    ...
    try {
      ...
      if (mlResult.available) {
        const isFraudDecision = !!mlResult.isFraud;
        const ruleBased = this.performRuleBasedAnalysis(jobText);
        const criticalPatterns = ['upfrontPayment', 'unrealisticSalary', 'suspiciousCommunication'];
        const hasCritical = (ruleBased.matchedFraudPatterns || []).some(p => criticalPatterns.includes(p.pattern) && p.matches && p.matches.length > 0);
        const finalIsFraud = isFraudDecision || hasCritical;
        console.log(`🎯 ML Decision: ${finalIsFraud ? '🚨 FRAUDULENT' : '✅ LEGITIMATE'}`);

        const reasons = mlResult.reasons ? mlResult.reasons.slice() : [
          `ML Model Prediction: ${finalIsFraud ? 'Fraudulent' : 'Legitimate'}`,
        ...
        if (hasCritical) {
          reasons.push('Critical fraud indicators detected: ' + (ruleBased.matchedFraudPatterns || [])
            .filter(p => criticalPatterns.includes(p.pattern))
            .map(p => p.pattern).join(', '));
        }

        finalResult = {
          isFraud: finalIsFraud,
          confidence: mlResult.confidence,
          riskLevel: this.calculateRiskLevel(mlResult.confidence, finalIsFraud),
  ...
  performRuleBasedAnalysis(jobText) {
    ...
    return {
      ...
      needsManualReview: !isFraud && riskLevel === 'medium'
    };
  }
